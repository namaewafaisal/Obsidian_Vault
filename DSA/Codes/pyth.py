...
# Paste your raw text inside the triple quotes below
raw_data = """
...
def convert_to_vlc_xml(text):
    header = '<?xml version="1.0" encoding="UTF-8"?>\n<Chapters>\n  <EditionEntry>\n'
    footer = '  </EditionEntry>\n</Chapters>'
    
    chapter_template = """    <ChapterAtom>
      <ChapterTimeStart>{time}.000</ChapterTimeStart>
      <ChapterDisplay>
...
"""
    ...
    body = ""
    # Regex to find HH:MM:SS and the title
    ...
    for line in text.strip().split('\n'):
        ...
        if match:
            ...
            title_str = match.group(2).replace("&", "&amp;") # Clean XML characters
            body += chapter_template.format(time=time_str, title=title_str)
            
    return header + body + footer

# Generate and save
xml_content = convert_to_vlc_xml(raw_data)
with open("java_chapters.xml", "w", encoding="utf-8") as f:
    f.write(xml_content)

print("Success! 'java_chapters.xml' has been created.")