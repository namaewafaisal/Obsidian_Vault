## Phase A — Arrays & Two Pointers (Implementation Fluency)

- [26. Remove Duplicates from Sorted Array](Problems/26.%20Remove%20Duplicates%20from%20Sorted%20Array.md)
- [27. Remove Element](27.%20Remove%20Element)
- [283. Move Zeroes](Problems/283.%20Move%20Zeroes.md)
- [977. Squares of a Sorted Array](Problems/977.%20Squares%20of%20a%20Sorted%20Array.md)
- [88. Merge Sorted Array](Problems/88.%20Merge%20Sorted%20Array.md)
- [189. Rotate Array](Problems/189.%20Rotate%20Array.md)
- [169. Majority Element](Problems/169.%20Majority%20Element.md) (re-implement without reference)

---

## Phase B — Hashing & Frequency (Easy → Medium Bridge)

- [217. Contains Duplicate](Problems/217.%20Contains%20Duplicate.md)
- [219. Contains Duplicate II](219.%20Contains%20Duplicate%20II)
- [1. Two Sum](Problems/1.%20Two%20Sum.md) (solve without hashmap first)
- [242. Valid Anagram](Problems/242.%20Valid%20Anagram.md)
- [383. Ransom Note](Problems/383.%20Ransom%20Note.md)
- [560. Subarray Sum Equals K](Problems/560.%20Subarray%20Sum%20Equals%20K.md) (re-do carefully)

---

## Phase C — Trees (Implementation-First, Before LeetCode)

### Core Implementations (Plain Java, no LeetCode first)
- Implement Binary Search Tree (insert, search)
- Inorder Traversal (recursive)
- Preorder Traversal (recursive)
- Postorder Traversal (recursive)
- Inorder Traversal (iterative)
- Find Inorder Predecessor in BST
- Find Inorder Successor in BST
- Delete a Node in BST (basic cases)

### Then LeetCode (Only After Implementation)
- [98. Validate Binary Search Tree](Problems/98.%20Validate%20Binary%20Search%20Tree.md)
- [235. Lowest Common Ancestor of a BST](Problems/235.%20Lowest%20Common%20Ancestor%20of%20a%20BST.md)
- [236. Lowest Common Ancestor of a Binary Tree](236.%20Lowest%20Common%20Ancestor%20of%20a%20Binary%20Tree)
- [105. Construct Binary Tree from Preorder and Inorder Traversal](Problems/105.%20Construct%20Binary%20Tree%20from%20Preorder%20and%20Inorder%20Traversal.md)

---

## Phase D — Sliding Window & Prefix (Reinforcement, Easy-Focused)

- [3. Longest Substring Without Repeating Characters](Problems/3.%20Longest%20Substring%20Without%20Repeating%20Characters.md)
- [424. Longest Repeating Character Replacement](Problems/424.%20Longest%20Repeating%20Character%20Replacement.md)
- [438. Find All Anagrams in a String](Problems/438.%20Find%20All%20Anagrams%20in%20a%20String.md)
- [567. Permutation in String](Problems/567.%20Permutation%20in%20String.md)
- [560. Subarray Sum Equals K](Problems/560.%20Subarray%20Sum%20Equals%20K.md)

---

## Phase E — Stack Fundamentals (After LC 84)

- [20. Valid Parentheses](Problems/20.%20Valid%20Parentheses.md)
- [150. Evaluate Reverse Polish Notation](Problems/150.%20Evaluate%20Reverse%20Polish%20Notation.md)
- [496. Next Greater Element I](496.%20Next%20Greater%20Element%20I)
- [739. Daily Temperatures](739.%20Daily%20Temperatures)
- [84. Largest Rectangle in Histogram](Problems/84.%20Largest%20Rectangle%20in%20Histogram.md) (review-only, no new code)

---

## Optional — Codeforces (Implementation Practice Only)

- Codeforces Div 3 — Problem A
- Codeforces Div 3 — Problem B
- Codeforces Div 2 — Problem A (optional)

Rules:
- No rating chasing
- No C/D problems
- Always write invariant after solve
