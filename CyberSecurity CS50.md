...
man in the middle - hacker posing as server to user and as user to server so data goes through them
Single sign on (SSO) - login to a site using the id from another site. Oauth
Password manager - store and manage passwords. passwords are encrypted using AES 256. master password
Passkeys - generated by the device as a 2 step verification. 